...
void Sound_Init(void){ unsigned long volatile delay;
  ...
  GPIO_PORTA_DIR_R |= 0x04;	// make PA2 out AND PA3 INPUT
  GPIO_PORTA_DIR_R &= ~0x08;
	// enable pull-up on PA3 (1 to enable pull up register))
  ...
  NVIC_ST_RELOAD_R = 90908;     // reload value for 500us //1.13636ms(assuming 80MHz) 90909-1
...
if(GPIO_PORTA_DATA_R&0x08&&(prev!=0x08))
	{
	if(quiet==1)
